  ...
  def gerarEscala(self):
    ...
    while len(pessoas) > 0:
      ...
      for pessoa in pessoas.copy():
        ...
        if len(desejos[pessoa]) == 0 and type(self.escala[pessoa]) != str:
...
prioridade = "Guilherme Vital Heltron Ribeiro Ramon Gleide Moisés Amanda_G Luiza Kariny Délio Amanda_V".split(" ")
desejos = {}
...
for d in desejos:
  temp = desejos[d].split(" ")
  temp = [i for i in temp if i != '']
  for idx, item in enumerate(temp):
    dia, turno = int(item[:-1]), item[-1]
    item = f'{dia}{turno}'
    temp[idx] = item
